...
class BLAST_Neighborhood
{
  ...
  NHResult infixRecursion(const std::string infix, const int score_threshold, const std::vector<std::vector<std::pair<std::string,int>>> & matrix) const;
  void buildMatrix(const ScoreMatrix& m, std::vector<std::vector<std::pair<std::string,int>>>& matrix);
  void sortMatrix(std::vector<std::vector<std::pair<std::string,int>>> & matrix);
  void sortNeighborhood(NHResult& neighborhood) const;
  std::vector<std::vector<std::pair<std::string,int>>> matrix;

  //const functions: function that does not change the state of the obejct
