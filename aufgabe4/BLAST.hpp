...
class BLAST_Neighborhood
{
  ...
  // hint: private member _variables_ are probably not required
    ...
    std::vector<std:string> generateInfixes(std::string query, int word_size);
    std::vector<std::string> generatePermutations(int counter, int word_size, std::string word);
